...
void ChatServer::Run( uint16 nPort, size_t max_players )
{
    ...
    for ( auto it: m_mapClients )
    {
        ...
        // to fl411		sprintf( nick, "Player%lu", 0 + ( rand() % m_game->getCurrentPlayers() +1 ) );ush this out and close gracefully.
...
void ChatServer::OnSteamNetConnectionStatusChanged( SteamNetConnectionStatusChangedCallback_t *pInfo )
{
	...
	// code really simple.
	char nick[ 64 ];
    long no_of_currently_joined_players = m_game->getCurrentn_Players() + 1;
